...
| 3a  | `/insights` aggregate query (original) | `Scan` + `FilterExpression` + `ProjectionExpression` | Lambda #3         | filter `review_status = auto_processed`; project only `category`, `urgency`, `sentiment`, `feature_tags`, `received_at` (data minimization — excludes `from_address`/`suggested_reply`/`raw_s3_key`) |
| 3b  | `/insights` tool-use query (parameterized) | `Scan` + `FilterExpression` + `ProjectionExpression` | Lambda #3 (via Bedrock tool use) | base filter `review_status = auto_processed` + optional filters on `category`, `sentiment`, `urgency`, `from_address`, `received_at` (date range); projects 9 fields: `email_id`, `from_address`, `subject`, `redacted_body`, `category`, `urgency`, `sentiment`, `feature_tags`, `received_at`. Bedrock may issue multiple Scans per request with different filters. |

**Why no GSI**: pattern #1 is a PK lookup (no index needed). Patterns #3a/#3b are `Scan`-based — sufficient at demo scale (doc02 tooling decision #4) and explicitly out of scope to optimize with a GSI (doc01 §6, "Out of Scope"). A GSI on `review_status` would speed up these patterns at higher record counts, but isn't justified here.
